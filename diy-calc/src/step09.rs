...
fn main() {
    ...
    if let Some(ref mut num) = input {
        *num = adjustment(*num, &buffer, bytes_read, &mut read_position);
    }
...
mod tests {
    ...
    fn test_01() {
        ...
        if let Some(ref mut num) = res {
            *num = adjustment(*num, &input, 2, &mut read_position);
        }
    ...
    fn test_02() {
        ...
        if let Some(ref mut num) = res {
            *num = adjustment(*num, &input, 6, &mut read_position);
        }
    ...
    fn test_03() {
        ...
        if let Some(ref mut num) = res {
            *num = adjustment(*num, &input, 10, &mut read_position);
        }
        assert_eq!(res, Some(-1));
    }
